...
async function main() {
  ...
  for (const item of ITEMS) {
    ...
    if (generic.category !== 'noise') {
      genericClassified.push({ ...item, category: generic.category, score: generic.score, why: generic.why })
    }
  ...
  logger.info({ genericPath, personalizedPath }, 'eval: snapshots written — diff and eyeball impactNote')
}
