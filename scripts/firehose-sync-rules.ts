...
async function main() {
  ...
  if (!apply) {
    ...
    for (const r of toUpdate) logger.info({ id: r.id, tag: r.tag, value: r.value }, 'sync: would update')
  ...
  logger.info(
    { created, updated, deleted, totalAfter, limit: ORG_RULE_LIMIT },
    'sync: done',
  )
}
...
async function listRules(token: string): Promise<FirehoseRule[]> {
  ...
  if (!res.ok) throw new Error(`GET /rules: HTTP ${res.status} — ${await res.text().catch(() => '')}`)
...
async function createRule(token: string, value: string, tag: string): Promise<FirehoseRule> {
  ...
  if (!res.ok) throw new Error(`POST /rules: HTTP ${res.status} — ${await res.text().catch(() => '')}`)
...
async function updateRule(
  ...
  if (!res.ok) throw new Error(`PUT /rules/${id}: HTTP ${res.status} — ${await res.text().catch(() => '')}`)
  const json = (await res.json()) as RuleMutationResponse
...
async function deleteRule(token: string, id: string): Promise<void> {
  ...
  if (!res.ok && res.status !== 204) {
    throw new Error(`DELETE /rules/${id}: HTTP ${res.status} — ${await res.text().catch(() => '')}`)
  }
