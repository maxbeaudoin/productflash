...
const { buildDigestItemRow, selectDiverseCandidates, startOfUtcDay } = await import(
  './synthesize'
)
...
describe('buildDigestItemRow', () => {
  ...
  test('meta found → row combines synthesized text with score/category/occurredAt from meta', () => {
    const byId = new Map([
      [
        'raw-1',
        { category: 'pricing', score: 78, publishedAt: new Date('2026-05-16T00:00:00Z') },
      ],
    ])
  ...
  test('publishedAt null propagates as null (no fabricated date — #41 rule)', () => {
    const byId = new Map([
      ['raw-1', { category: 'feature', score: 50, publishedAt: null }],
    ])
    const row = buildDigestItemRow('user-1', synthesized, byId)
