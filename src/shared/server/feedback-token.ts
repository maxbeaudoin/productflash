...
function compute(digestItemId: string, rating: "up" | "down"): string {
  ...
  return createHmac("sha256", secret).update(`${digestItemId}.${rating}`).digest("base64url");
}

export function signFeedbackToken(digestItemId: string, rating: "up" | "down"): string {
