...
function inputItem(rawItemId: string, overrides: Partial<SynthesisInputItem> = {}): SynthesisInputItem {
  return {
    rawItemId,
...
describe('synthesizeDigest — output validation', () => {
  ...
  test('empty headline is rejected', async () => {
    ...
    await expect(
      synthesizeDigest({ ...BASE_INPUT, items: [inputItem('raw-1')] }),
    ).rejects.toThrow(/invalid headline/)
  })
  ...
  test('empty impactNote is rejected', async () => {
    ...
    await expect(
      synthesizeDigest({ ...BASE_INPUT, items: [inputItem('raw-1')] }),
    ).rejects.toThrow(/invalid impactNote/)
  })
