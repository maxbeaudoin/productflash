...
const SYSTEM_PROMPT = [
  ...
  "  2. RECORD FIRST, EXPLORE SECOND. Iteration 2's job is to call record_source — in parallel, one per source — for every URL from the homepage links list whose path alone is a known signal. DO NOT fetch_page these first; the path IS the evidence. Record at minimum:",
  ...
  "     • Roadmap, changelog, releases, updates, announcements, news, press — 'webpage' (list pages).",
  "     • Blog — 'webpage'.",
  "     • Social profiles — anything matching linkedin.com, twitter.com, x.com, youtube.com. Record straight from the links list; socials are recorded inert.",
  "    By the end of iteration 2 you should have 4+ record_source calls in flight. If you finish iteration 2 with zero records, you are doing it wrong.",
  "  3. AMBIGUOUS-ONLY verification: fetch_page is ONLY for paths that aren't self-evident (e.g. /resources, /solutions — could be article list or generic landing). Never fetch_page a self-evident path (/pricing, /blog, /changelog, /press, /<product-name>, social URLs) — recording it directly is the whole point of step 2.",
