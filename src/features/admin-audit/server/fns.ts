...
import type { AdminAuditPayload, AdminAuditRow } from "../shared/types";
...
// NOTE: only createServerFn-wrapped exports live here on purpose. Plain
// async helpers like the original `loadAuditForTarget` keep their imports
// (`~/db/schema`, `~/shared/server/db`) reachable from the client bundle
// because TanStack Start's Vite plugin only strips bodies it knows are
// server-only. Per-target queries that need to run inside a route loader
// are inlined at the callsite (see `loadUserDetail` in
// `routes/admin/users/$userId.tsx`) — duplication is cheaper than the
// pg-into-client-bundle leak we got the first time around.
