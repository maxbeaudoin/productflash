...
export async function autodetectRSSForHomepage(
  ...
  try {
    ...
    logger.debug({ err, homepageUrl }, 'rss autodetect: homepage fetch failed, falling back to path probe')
  }
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      if ((feed.items?.length ?? 0) > 0) {
        logger.info({ homepageUrl, feedUrl: candidate, items: feed.items!.length }, 'rss autodetect: resolved')
        return candidate
      }
    } catch {
