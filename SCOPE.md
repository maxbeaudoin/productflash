...
- **Single pillar**: competitor moves only — launches, pricing changes, feature releases, positioning shifts, plus funding rounds and acquisitions when they surface via the competitor-moves sources (labelling refinement, not a new sourcing pillar — see "out of scope" below).
...
- Market signal pillar as a dedicated sourcing track (analyst coverage, Crunchbase/PitchBook-style funding feeds, etc.). Note: funding/acquisition news that incidentally arrives via the competitor-moves sources is labelled as such and included — see "in scope" — but we are not adding new sources or pipelines to chase market-signal stories.
