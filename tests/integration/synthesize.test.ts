...
describe('runSynthesisForUser — F-003 happy path', () => {
  test('non-empty pool → digest row + digest_items rows + llm_usage row', async () => {
    ...
    expect(items.map((i) => i.headline).sort()).toEqual([
      'H: New feature',
      'H: Pricing change',
    ])
  })
...
describe('runSynthesisForUser — F-009 tenant isolation', () => {
  test('running for user A leaves user B untouched', async () => {
    ...
    const aItems = await h.db
      .select()
      .from(digestItems)
      .where(eq(digestItems.userId, a.userId))
    ...
    const bItems = await h.db
      .select()
      .from(digestItems)
      .where(eq(digestItems.userId, b.userId))
    ...
    const bRaws = await h.db
      ...
      .from(rawItems)
      .innerJoin(itemScores, and(eq(itemScores.rawItemId, rawItems.id), eq(itemScores.userId, b.userId)))
  ...
  test("A's noise-classified items do NOT leak into B's digest", async () => {
    ...
    const aItems = await h.db
      .select()
      .from(digestItems)
      .where(eq(digestItems.userId, userA!.id))
    ...
    const bScores = await h.db
      .select()
      .from(itemScores)
      .where(eq(itemScores.userId, userB!.id))
